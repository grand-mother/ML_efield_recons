...
# root_dir = os.path.realpath(os.path.join(os.path.split(__file__)[0],
#                                          "../radio-simus")) # = $PROJECT
...
# if no outfilename is given, it will store the output in table AntennaInfo,
# in the same HDF5 file, as a new column.
# (TODO: handle what happens if this already exists)

# this computes the peak to peak amplitudes of all the traces
# (and if voltage is available in the voltages, etc)

# TODO: Split this in 3 functions: GetP2PFromTrace(Trace)
# to get p2px,p2py,p2pz and p2ptotal
# used in GetP2P(InputFilename, CurrentEventName,AntennaID,tracetype)
# to get the proper trace
# used in GetEventP2P(InputFilename, CurrentEventName)
# to get them for all the antennas in the event


def get_p2p_hdf5(InputFilename, antennamax='All', antennamin=0,
                 usetrace='efield'):
    """
    Output the peak to peak electric field and amplitude.
    Read in all traces from antennamax to antennamin
    Parameters_:
    InputFilename: str
        HDF5File
    antennamin: int
    ...
    Output:
    p2pE: numpy array
        [p2p_Ex, p2p_Ey, p2p_Ez, p2p_total]: peak-to-peak electric fields
        along x, y, z, and norm
    """
    CurrentRunInfo = hdf5io.GetRunInfo(InputFilename)
    # using the first event of each file (there is only one for now)
    ...
    for i in range(antennamin, antennamax+1):
        ...
        if(usetrace == 'efield'):
            trace = hdf5io.GetAntennaEfield(InputFilename, CurrentEventName,
                                            AntennaID, OutputFormat="numpy")
        elif(usetrace == 'voltage'):
            trace = hdf5io.GetAntennaVoltage(InputFilename, CurrentEventName,
                                             AntennaID, OutputFormat="numpy")
        elif(usetrace == 'filteredvoltage'):
            trace = hdf5io.GetAntennaFilteredVoltage(InputFilename,
                                                     CurrentEventName,
                                                     AntennaID,
                                                     OutputFormat="numpy")
        elif(usetrace == 'filteredvoltagenoise'):
            trace = hdf5io.GetAntennaFilteredVoltageNoise(InputFilename,
                                                          CurrentEventName,
                                                          AntennaID,
                                                          OutputFormat="numpy")
        else:
            print('You must specify either efield, voltage, filteredvoltage,'
                  + 'or filteredvoltagenoise, bailing out')
        ...
        # combined components
        # amplitude = np.sqrt(trace.T[1]**2. + trace.T[2]**2. + trace.T[3]**2.)
