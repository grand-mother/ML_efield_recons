...
path = '/Users/claireguepin/Projects/GRAND/'\
    + 'ML_efield_recons/Recons_Peak_Epmin1EeV_LossCustom/'
...
# ax.set_title(r"Histogram peak time index", fontsize=14)
...
# ax.yaxis.set_ticks_position('both')
# ax.tick_params(labelsize=14)
# ax.set_title(r"Histogram log10 peak amplitude", fontsize=14)
...
indobj = 10  # 5ns -> 10 timesteps because 0.5ns sampling initially
valobj = 0.10  # 5-10%
...
for train_sig in train_loader:
    ...
    MSE += nant*(indobj/indmax)**2 + nant*np.log10(1.+valobj)**2
    MSE_ind += nant*(indobj/indmax)**2
    MSE_amp += nant*np.log10(1.+valobj)**2
MSE /= it_train
MSE_ind /= it_train
MSE_amp /= it_train
...
def my_loss(output, target):
    ...
    loss_ind = torch.mean((output[:, 0] - target[:, 0])**2)\
        / (indobj/indmax)**2
    loss_amp = torch.mean((output[:, 1] - target[:, 1])**2)\
        / np.log10(1.+valobj)**2
    loss = (loss_ind + loss_amp)/2.
    return loss
...
# TEST which is smaller for more constraining MSE
# x = np.arange(0.1, 0.9, 0.01)
# fig = plt.figure()
# ax = plt.gca()
# plt.subplots_adjust(left=0.13)
# plt.plot(x, np.log10(1-x)**2, ls='-', lw=2)
# plt.plot(x, np.log10(1+x)**2, ls='--', lw=2)
# ax.tick_params(labelsize=14)
# ax.xaxis.set_ticks_position('both')
# ax.yaxis.set_ticks_position('both')
...
def vis_hist(data_loader, namefig):
    ...
    ax.set_title(r"Histogram Time (t_{\rm real}-t_{\rm reco})"
                 + "\n"+r"Mean: %.1f, Std: %.1f" % (mean, std), fontsize=14)
    # ax.legend(prop={'size': 12}, frameon=False)
    plt.savefig(path+"Hist_tim_"+namefig+".pdf")
    ...
    ax.yaxis.set_ticks_position('both')
    ax.tick_params(labelsize=14)
    mean = np.mean((10**val_real-10**val_reco)/10**val_real)
    std = np.std((10**val_real-10**val_reco)/10**val_real)
    ax.set_title(r"Histogram Amplitude"
                 + r"(A_{\rm real}-A_{\rm reco})/A_{\rm real}"
                 + "\n"+r"Mean: %.1e, Std: %.1e" % (mean, std), fontsize=14)
    # ax.legend(prop={'size': 12}, frameon=False)
    plt.savefig(path+"Hist_amp_"+namefig+".pdf")
